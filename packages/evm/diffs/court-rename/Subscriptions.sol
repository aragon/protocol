...
interface IAragonAppFeesCashier {
    ...
    */
    event AppFeeSet(bytes32 indexed appId, IERC20 token, uint256 amount);
    ...
    */
    function setAppFee(bytes32 _appId, IERC20 _token, uint256 _amount) external;
    ...
    */
    function setAppFees(bytes32[] calldata _appIds, IERC20[] calldata _tokens, uint256[] calldata _amounts) external;
    ...
    */
    function getAppFee(bytes32 _appId) external view returns (IERC20 token, uint256 amount);
}
...
// File: contracts/subscriptions/ProtocolSubscriptions.sol
...
contract ProtocolSubscriptions is ControlledRecoverable, TimeHelpers, ISubscriptions, IAragonAppFeesCashier {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    ...
    string private constant ERROR_guardian_FEES_ALREADY_CLAIMED = "CS_guardian_FEES_ALREADY_CLAIMED";
    string private constant ERROR_guardian_NOTHING_TO_CLAIM = "CS_guardian_NOTHING_TO_CLAIM";
    string private constant ERROR_DONATION_AMOUNT_ZERO = "CS_DONATION_AMOUNT_ZERO";
    string private constant ERROR_protocol_HAS_NOT_STARTED = "CS_protocol_HAS_NOT_STARTED";
    ...
    struct Period {
        uint64 balanceCheckpoint;               // Protocol term ID of a period used to fetch the total active balance of the guardians registry
        IERC20 feeToken;                        // Fee token used for the subscription fees corresponding to a certain period
        uint256 totalActiveBalance;             // Total amount of guardian tokens active in the Protocol at the corresponding period checkpoint
        uint256 collectedFees;                  // Total amount of subscription fees collected during a period
        uint256 accumulatedGovernorFees;        // Total amount of fees accumulated for the governor of the Protocol during a period
        mapping (address => bool) claimedFees;  // List of guardians that have claimed fees during a period, indexed by guardian address
    }

    // Duration of a subscription period in Protocol terms
    uint64 public periodDuration;

    // Permyriad of subscription fees that will be allocated to the governor of the Protocol (‱ - 1/10,000)
    uint16 public governorSharePct;

    // ERC20 token used for the subscription fees
    IERC20 public currentFeeToken;
    ...
    event FeesDonated(address indexed payer, uint256 indexed periodId, IERC20 feeToken, uint256 feeAmount);
    event FeesClaimed(address indexed guardian, uint256 indexed periodId, IERC20 feeToken, uint256 guardianShare);
    event GovernorFeesTransferred(IERC20 indexed feeToken, uint256 amount);
    event FeeTokenChanged(IERC20 previousFeeToken, IERC20 currentFeeToken);
    event GovernorSharePctChanged(uint16 previousGovernorSharePct, uint16 currentGovernorSharePct);

    /**
    * @dev Initialize protocol subscriptions
    * @param _controller Address of the controller
    * @param _periodDuration Duration of a subscription period in Protocol terms
    * @param _feeToken Initial ERC20 token used for the subscription fees
    * @param _governorSharePct Initial permyriad of subscription fees that will be allocated to the governor of the Protocol (‱ - 1/10,000)
    */
    constructor(Controller _controller, uint64 _periodDuration, IERC20 _feeToken, uint16 _governorSharePct)
    ...
    /**
    * @notice Donate fees to the Protocol
    ...
    */
    function donate(uint256 _amount) external payable {
        ...
        (uint256 currentPeriodId, Period storage period, IERC20 feeToken) = _ensureCurrentPeriodFeeToken();
    ...
    */
    function claimFees(uint256 _periodId) external {
        ...
        require(!period.claimedFees[msg.sender], ERROR_guardian_FEES_ALREADY_CLAIMED);

        // Check claiming guardian has share fees to be transferred
        (uint64 periodBalanceCheckpoint, uint256 totalActiveBalance) = _ensurePeriodBalanceDetails(_periodId, period);
        uint256 guardianShare = _getGuardianShare(msg.sender, period, periodBalanceCheckpoint, totalActiveBalance);
        require(guardianShare > 0, ERROR_guardian_NOTHING_TO_CLAIM);

        // Update guardian state and transfer share fees
        period.claimedFees[msg.sender] = true;
        IERC20 feeToken = _ensurePeriodFeeToken(period);
        _transfer(msg.sender, feeToken, guardianShare);
        emit FeesClaimed(msg.sender, _periodId, feeToken, guardianShare);
    }
    ...
    */
    function setFeeToken(IERC20 _feeToken) external onlyConfigGovernor {
        // If the protocol has started, ensure the fee token is set for the current period before changing it
    ...
    * @param _governorSharePct New permyriad of subscription fees that will be allocated to the governor of the Protocol (‱ - 1/10,000)
    */
    ...
    */
    function setAppFee(bytes32 _appId, IERC20 _token, uint256 _amount) external onlyConfigGovernor {
        // Ensure fee token data for the current period
        (,, IERC20 feeToken) = _ensureCurrentPeriodFeeToken();
    ...
    */
    function setAppFees(bytes32[] calldata _appIds, IERC20[] calldata _tokens, uint256[] calldata _amounts) external onlyConfigGovernor {
    ...
    * @dev To be called by the eventual dispute creator (usually an IArbitrable). If a fee is defined, it requires the sender to have pre-approved tokens.
           We expect the caller to submit the correct appId here, and leave it to guardians to verify that the correct fees were paid ahead of disputes.
    * @param _appId Id of the app paying fees for
    * @param _data Extra data for context of the payment
    */
    function payAppFees(bytes32 _appId, bytes calldata _data) external payable {
        // Ensure fee token data for the current period
        (,Period storage period, IERC20 feeToken) = _ensureCurrentPeriodFeeToken();
    ...
    * @return accumulatedGovernorFees Total amount of fees accumulated for the governor of the Protocol during a period
    */
    ...
    * @return newLastPeriodId Identification number of the resulting last paid period
    */
    function getOwedFeesDetails(address /*_subscriber*/) external view returns (IERC20 feeToken, uint256 amountToPay, uint256 newLastPeriodId) {
        ...
        // However, we still need to support this view function to be compliant with the AragonProtocol's IArbitrator implementation.
    ...
    /**
    * @dev Tell the share fees corresponding to a guardian for a certain period
    * @param _guardian Address of the guardian querying the owed shared fees of
    ...
    * @return guardianShare Amount of share fees owed to the given guardian for the requested period
    */
    function getGuardianShare(address _guardian, uint256 _periodId) external view returns (IERC20 feeToken, uint256 guardianShare) {
        require(_periodId < _getCurrentPeriodId(), ERROR_NON_PAST_PERIOD);
        Period storage period = periods[_periodId];
        ...
        // Compute guardian share fees using the period balance details
        guardianShare = _getGuardianShare(_guardian, period, periodBalanceCheckpoint, totalActiveBalance);
        feeToken = _getEnsuredPeriodFeeToken(period);
    }

    /**
    * @dev Check if a given guardian has already claimed the owed share fees for a certain period
    * @param _guardian Address of the guardian being queried
    * @param _periodId Identification number of the period being queried
    * @return True if the owed share fees have already been claimed, false otherwise
    */
    function hasGuardianClaimed(address _guardian, uint256 _periodId) external view returns (bool) {
        return periods[_periodId].claimedFees[_guardian];
    }
    ...
    */
    function getAppFee(bytes32 _appId) external view returns (IERC20 feeToken, uint256 feeAmount) {
        (, Period storage period) = _getCurrentPeriod();
        feeToken = _getEnsuredPeriodFeeToken(period);
    ...
    */
    function _transferFeesToGovernor(Period storage _period) internal {
        ...
        IERC20 feeToken = _ensurePeriodFeeToken(_period);
    ...
    */
    function _deposit(address _from, IERC20 _token, uint256 _amount) internal {
        if (_amount == 0) {
            return;
        }
    ...
    */
    function _transfer(address payable _to, IERC20 _token, uint256 _amount) internal {
        if (_amount == 0) {
            return;
        }
    ...
    */
    function _ensureCurrentPeriodFeeToken() internal returns (uint256 periodId, Period storage period, IERC20 feeToken) {
    ...
    */
    function _ensurePeriodFeeToken(Period storage _period) internal returns (IERC20) {
        // Use current fee token address for the given period if these haven't been set yet
        IERC20 feeToken = _period.feeToken;
        if (feeToken == IERC20(0)) {
    ...
    * @param _feeToken New ERC20 token to be used for the subscription fees
    */
    function _setFeeToken(IERC20 _feeToken) internal {
    ...
    */
    function _setAppFee(bytes32 _appId, IERC20 _token, uint256 _amount) internal {
    ...
    */
    function _getCurrentPeriodId() internal view returns (uint256) {
        // Since the Protocol starts at term #1, and the first subscription period is #0, then subtract one unit to the current term of the Protocol
    ...
    * @return ERC20 token to be used for the subscription fees during the given period
    */
    function _getEnsuredPeriodFeeToken(Period storage _period) internal view returns (IERC20) {
        // Return current fee token address and amount if these haven't been set for the given period yet
        IERC20 feeToken = _period.feeToken;
        return feeToken == IERC20(0) ? currentFeeToken : feeToken;
    }

    /**
    * @dev Internal function to get the total active balance of the guardians registry at a random term during a period
    * @param _periodId Identification number of the period being queried
    * @return periodBalanceCheckpoint Protocol term ID used to fetch the total active balance of the guardians registry
    * @return totalActiveBalance Total amount of guardian tokens active in the Protocol at the corresponding used checkpoint
    */
    ...
    /**
    * @dev Internal function to tell the share fees corresponding to a guardian for a certain period
    * @param _guardian Address of the guardian querying the owed shared fees of
    * @param _period Period being queried
    * @param _periodBalanceCheckpoint Protocol term ID used to fetch the active balance of the guardian for the requested period
    * @param _totalActiveBalance Total amount of guardian tokens active in the Protocol at the corresponding used checkpoint
    * @return Amount of share fees owed to the given guardian for the requested period
    */
    function _getGuardianShare(address _guardian, Period storage _period, uint64 _periodBalanceCheckpoint, uint256 _totalActiveBalance)
        internal
        view
        returns (uint256)
    {
        // Fetch guardian active balance at the checkpoint used for the requested period
        IGuardiansRegistry guardiansRegistry = _guardiansRegistry();
        uint256 guardianActiveBalance = guardiansRegistry.activeBalanceOfAt(_guardian, _periodBalanceCheckpoint);
        if (guardianActiveBalance == 0) {
            return 0;
        }

        // Note that we already checked the guardian active balance is greater than zero, then, the total active balance must be greater than zero.
        return _period.collectedFees.mul(guardianActiveBalance) / _totalActiveBalance;
    }
}
