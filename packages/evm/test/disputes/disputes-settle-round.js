        ...
        const itSettlesPenaltiesAndRewardsProperly = (roundId, expectedWinningGuardians, expectedLosingGuardians) => {
          ...
          beforeEach('load previous balances', async () => {
            ...
            for (const { address } of guardians) {
              const { active, available, locked } = await courtHelper.guardiansRegistry.detailedBalanceOf(address)
              previousBalances[address] = { active, available, locked }
            }

            const { active, available, locked } = await courtHelper.guardiansRegistry.detailedBalanceOf(BURN_ADDRESS)
            ...
            const itSettlesPenaltiesProperly = () => {
              it('unlocks the locked balances of the winning guardians and slashes the losing ones', async () => {
                for (const { address } of expectedWinningGuardians) {
                  ...
                  const { active: currentActiveBalance, locked: currentLockedBalance } = await courtHelper.guardiansRegistry.detailedBalanceOf(address)
              ...
              it('burns the collected tokens if necessary', async () => {
                ...
                const { available: currentAvailableBalance } = await courtHelper.guardiansRegistry.detailedBalanceOf(BURN_ADDRESS)
              ...
              if (expectedWinningGuardians.length > 0) {
                ...
                it('rewards the winning guardians with guardian tokens and fees', async () => {
                  ...
                  for (const { address, weight } of expectedWinningGuardians) {
                    ...
                    const { available } = await courtHelper.guardiansRegistry.detailedBalanceOf(address)
                ...
                if (roundId >= maxRegularAppealRounds.toNumber()) {
                  it('locks only after final round lock period', async () => {
                    ...
                    for (const guardian of expectedWinningGuardians) {
                      ...
                      await courtHelper.guardiansRegistry.deactivate(guardian.address, 0, { from: guardian.address }) // deactivate all
                    }

                    // fails to withdraw on next term
                    await courtHelper.passTerms(bn(1))
                    for (const guardian of expectedWinningGuardians) {
                      await assertRevert(courtHelper.guardiansRegistry.unstake(guardian.address, amount, { from: guardian.address }), REGISTRY_ERRORS.WITHDRAWALS_LOCK)
                    }
                    ...
                    await courtHelper.passTerms(bn(1))
                    for (const guardian of expectedWinningGuardians) {
                      const receipt = await courtHelper.guardiansRegistry.unstake(guardian.address, amount, { from: guardian.address })
